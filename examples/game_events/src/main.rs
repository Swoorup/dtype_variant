...
impl EventProcessor {
    ...
    fn process_event(&mut self, event: &GameEvent) {
        ...
        println!("{} {} - {}: {:?}", priority, category, 
                 self.get_event_type_name(event), event);
        ...
        if let Some(death) = event.downcast_ref::<PlayerDeathVariant>() {
            ...
            println!("  → Player {} died: {} (Total deaths: {})", 
        ...
        if let Some(item_id) = event.downcast_ref::<ItemPickupVariant>() {
            ...
            println!("  → Item {} collected (Total items: {})", 
                     item_id, self.items_collected.len());
        }
        ...
        match event {
            GameEvent::PlayerConnect => {
                ...
                println!("  → Player connected (Online: {})", self.connected_players);
            },
            GameEvent::PlayerDisconnect => {
                self.connected_players = self.connected_players.saturating_sub(1);
                println!("  → Player disconnected (Online: {})", self.connected_players);
            },
            GameEvent::ServerCrash => {
                println!("  → 💥 SERVER CRASH DETECTED! Initiating emergency procedures...");
    ...
    fn emergency_shutdown(&mut self) {
        ...
        println!("    💾 Backing up chat logs ({} messages)...", self.chat_messages.len());
        println!("    🔒 Securing {} connected players...", self.connected_players);
    ...
    fn print_summary(&self) {
        ...
        if !self.chat_messages.is_empty() {
            println!("💭 Recent chats: {:?}", &self.chat_messages[..self.chat_messages.len().min(3)]);
        }
        
        if !self.items_collected.is_empty() {
            println!("🎯 Items found: {:?}", &self.items_collected[..self.items_collected.len().min(5)]);
        }
    }
}
...
fn main() {
    ...
    let mut events = vec![
        ...
        GameEvent::PlayerMove { player_id: 2, x: 5.0, y: 15.0 },
        ...
        GameEvent::PlayerMove { player_id: 1, x: 12.0, y: 18.5 },
    ...
    println!("\n🔍 === Event Analysis Demo ===");
    let test_events = [
        GameEvent::PlayerMove { player_id: 99, x: 100.0, y: 200.0 },
